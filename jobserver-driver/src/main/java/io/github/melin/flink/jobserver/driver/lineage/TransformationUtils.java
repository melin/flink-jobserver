...
public class TransformationUtils {
    ...
    public List<SinkLineage> convertToVisitable(List<Transformation<?>> transformations) {
        ...
        for (Transformation<?> transformation : transformations) {
            Optional<SinkLineage> sinkLineage = processSink(transformation);
    ...
    public Optional<SinkLineage> processSink(Transformation<?> transformation) {
        log.info("sink transformation: {}" + transformation.getClass().getName());

        List<Transformation<?>> predecessors = transformation.getTransitivePredecessors();
        for (Transformation<?> predecessor : predecessors) {
            log.info("source transformation: {}" + predecessor.getClass().getName());
        }
        ...
        predecessors = transformation.getTransitivePredecessors();
        for (Transformation<?> predecessor : predecessors) {
            Optional<Object> source = processSource(predecessor);
    ...
    public Optional<Object> processSource(Transformation<?> transformation) {
        log.info("source transformation: {}" + transformation.getClass().getName());
