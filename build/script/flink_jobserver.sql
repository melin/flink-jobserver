...
CREATE TABLE `fjs_cluster` (
  ...
  `flink_app_config` longtext COMMENT 'flink config',
  `flink_session_config` longtext COMMENT 'flink session config',
  `flink_session_enabled` smallint DEFAULT NULL COMMENT '是否启动flink session',
...
CREATE TABLE `fjs_flink_driver` (
  ...
  `deploment_mode` varchar(32) DEFAULT NULL COMMENT '部署模式：application & session',
...
CREATE TABLE `fjs_job_instance` (
  ...
  `runtime_mode` varchar(32) DEFAULT NULL COMMENT '运行模式：batch, stream',
  `deployment_mode` varchar(32) DEFAULT NULL COMMENT '部署模式：application & session',
